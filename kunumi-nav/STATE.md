...
Every row is a deployed build. `?v` = the cache-bust number in the live URL; `commit` is in the `studies` repo.
Restore any: `cd ~/CLAUDE/projects/studies && git show <commit>:kunumi-nav/index.html > ~/CLAUDE/projects/prototypes/kunumi-nav/index.html`

| `?v` | commit | milestone |
|---|---|---|
| 57 | `457974e` | Initial — Nevverland organic nav in the Kunumi finish (centered radial type + dark threads + starfield) |
| 58 | `7d46f59` | Focus copy block, field threads, project imagery (ring-bloom), nav-aside stage |
| 59 | `6f8c1b8` | Fix: `#tiles` overlay was intercepting all clicks |
| 60 | `9c06427` | Less-springy defaults + live controls panel (**C**), abrupt nav shift, bigger stage |
| **61** | `49a64c1` | ★ **Big rework** — category/leaf become Kunumi rollover image fields; click image → surface item / zoom stage in place; idle nav wander |
| 62 | `55488c0` | Clickable client text call-outs in categories + more (duped) floating imagery |
| 63 | `38b81b9` | Perf — transform-positioned tiles (no layout thrash), skip invisible |
| 64 | `e8f33db` | Perf — ★ lazy-mount 1024px images only when revealed (fixed Work-entry lag) |
| 65 | `ba2b080` | Clear-zone (images fade from under the nav) + nav type shrinks in field mode |
| 66 | `f0e657f` | ⊘ Landing hover-preview — **REVERTED by request; do not re-add without asking** |
| 67 | `b08f81e` | Rolled back to v65 + first STATE.md |
| 68 | `d34e412` | Denser field + project thumbnails larger & semi-visible at rest |
| 69 | `972c197` | Denser again (470 dots) + landing sub-nav at half opacity |
